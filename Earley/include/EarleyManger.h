...
class EarleyManager {
 private:
  struct Situation {   //  [src -> w1w2, parent_pos], when |w1| = point_pos
    char src;
    size_t point_pos;  //  pos of the letter, which is after point (size if * is last symbol)
    std::string word;  //  = ab
    size_t parent_pos;
    Situation() = default;
    Situation(char src, std::string word, size_t point_pos, size_t parent_pos)
    : src(src), word(word), point_pos(point_pos), parent_pos(parent_pos) {}
  ...
  struct Situation_hash
  {
    ...
    {
      return std::hash<std::string>()(sit.word) ^ sit.parent_pos ^ sit.point_pos;
    }
  };

  struct Backet {
    Backet(const Situation&);
